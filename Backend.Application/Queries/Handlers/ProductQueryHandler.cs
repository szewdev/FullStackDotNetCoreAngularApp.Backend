...
public class ProductQueryHandler(IProductRepository context) : IProductQueryHandler
{
    ...
    public async Task<ProductDto?> Handle(GetProductQuery query)
    {
        ...
        return p != null ? new ProductDto(p.Id, p.Name, p.Price, p.Description, p.IsActive) : null;
    }

    public async Task<List<ProductDto>> Handle()
    {
        var products = await _repository.GetAllProducts();
        return products.Select(x => new ProductDto(x.Id, x.Name, x.Price, x.Description, x.IsActive)).ToList();
    }
}
